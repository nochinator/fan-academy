...
export class Banner extends Phaser.GameObjects.Container {
  ...
  constructor(context: GameScene, board: Board, playerData: IUserData[]) {
    ...
    this.playerOnePortrait = context.add.image(0, 0, playerData[0].username).setOrigin(0.5).setPosition(600, 73).setScale(0.3).setDepth(1);
    this.playerTwoPortrait = context.add.image(0, 0, playerData[1].username).setOrigin(0.5).setPosition(1200, 73).setScale(0.3).setFlipX(true).setDepth(1);
